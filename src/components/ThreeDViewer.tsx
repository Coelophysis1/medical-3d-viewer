...
export default function ThreeDViewer({ models, onVolumesLoaded }: ThreeDViewerProps) {
  ...
  const bgColorsRef = useRef([
    ...
    ['#000000', '#808080', '#f0f0f0'],  // 电影模式背景色（黑/灰/淡灰）
  ]);
  ...
  const setupScene = useCallback((container: HTMLDivElement) => {
    ...
    // 补光
    const fillLight = new THREE.DirectionalLight(0xc8d8e8, 2.0);
    ...
    const loadModels = async () => {
      ...
      if (onVolumesLoaded) {
        const dims = newMeshes.map(m => {
          ...
          return {
            x: Math.round(((bb.max.x - bb.min.x) / 10) * 100) / 100,
            y: Math.round(((bb.max.y - bb.min.y) / 10) * 100) / 100,
            z: Math.round(((bb.max.z - bb.min.z) / 10) * 100) / 100,
          };
        });
  ...
  const handleToggleRenderMode = useCallback(() => {
    ...
    if (nextMode === 'classic') {
      ...
      s.renderer.toneMappingExposure = 2.0;
      s.scene.environment = null;
      s.keyLight.intensity = 3.5;
      s.fillLight.intensity = 2.0;
      s.rimLight.intensity = 1.0;
      s.ambientLight.intensity = 0.8;
